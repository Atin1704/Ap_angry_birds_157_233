...
//        pigs.add(new Stone_block(world, bodyRemovalManager, 865, 380, 110, 110));
    ...
    public void render(float delta) {
        ...
        if (allPigBodiesNull() && allBirdsLaunched()) {
            game_runner.setScreen(new Victory_Screen(game_runner, assetManager, 3));
        } else if (!allPigBodiesNull() && allBirdsLaunched()) {
            game_runner.setScreen(new Defeat_Screen(game_runner, assetManager, 3));
        } else if (allPigBodiesNull() && !allBirdsLaunched()) {
            game_runner.setScreen(new Victory_Screen(game_runner, assetManager, 3));
        }
        ...
        if (Gdx.input.justTouched()) {
            ...
            if (touchPos.x >= pauseButtonX && touchPos.x <= (pauseButtonX + pauseButtonWidth)
                ...
                Timer.schedule(new Timer.Task() {
                    @Override
                    public void run() {
                        game_runner.setScreen(new pause_screen(game_runner, assetManager, 3,currentScreen));
                    }
